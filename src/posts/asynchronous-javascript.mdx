...
tags: [Javascript]
cover: './covers/asynchronous-javascript.jpg'
---

## 동기식 처리 모델과 비동기식 처리 모델 {#synchronous-and-asynchronous-model}
...
## 자바스크립트 실행 환경 {#javascript-runtime}
...
![자바스크립트 실행 환경](./images/asynchronous-javascript-1.png)
...
### 동기식 처리 모델 예시 {#examples-of-synchronous-model}
...
![동기식 처리 모델에서의 호출 스택](./images/asynchronous-javascript-2.png)

### 비동기식 처리 모델 예시 {#examples-of-asynchronous-model}
...
## 비동기 프로그래밍 기법 {#asynchronous-programming-pattern}
...
### 콜백 패턴의 문제점 {#problems-with-callback-patterns}

콜백 패턴은 자바스크립트 개발자들 사이에서 비동기 프로그래밍을 위해 널리 사용되는 패턴이었지만, 몇 가지 치명적인 문제점을 갖고 있었다.

#### 콜백 지옥(Callback Hell) {#callback-hell}
...
#### 에러 처리의 한계 {#limitations-of-error-handling}
...
### 프로미스가 해결한 문제들 {#problems-solved-by-promise}
...
### 프로미스 패턴 활용 예시 {#examples-of-the-promise-pattern}
...
## 비동기 함수(Async Function) {#async-function}
